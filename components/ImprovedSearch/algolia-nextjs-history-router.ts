...
type CreateURL<TRouteState> = (args: { qsModule: typeof qs; routeState: TRouteState; location: Location }) => string

type ParseURL<TRouteState> = (args: { qsModule: typeof qs; location: Location }) => TRouteState
...
class BrowserHistory<TRouteState> implements Router<TRouteState> {
   ...
   * @default 400
   */
  private readonly writeDelay: Required<BrowserHistoryArgs<TRouteState>>["writeDelay"]
  /**
   * Creates a full URL based on the route state.
   * The storage adaptor maps all syncable keys to the query string of the URL.
   */
  private readonly _createURL: Required<BrowserHistoryArgs<TRouteState>>["createURL"]
  /**
   * Parses the URL into a route state.
   * It should be symmetrical to `createURL`.
   */
  private readonly parseURL: Required<BrowserHistoryArgs<TRouteState>>["parseURL"]
  /**
   * Returns the location to store in the history.
   * @default () => window.location
   */
  private readonly getLocation: Required<BrowserHistoryArgs<TRouteState>>["getLocation"]
